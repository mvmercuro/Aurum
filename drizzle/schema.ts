...
export const products = mysqlTable("products", {
  ...
  isActive: boolean("isActive").notNull().default(true),
  inventoryCount: int("inventoryCount").notNull().default(0),
...
export const orders = mysqlTable("orders", {
  ...
  paymentMethod: mysqlEnum("paymentMethod", ["cash", "debit"]).notNull(),
  // Customer info
...
export const orderItems = mysqlTable("orderItems", {
  ...
  priceCentsAtPurchase: int("priceCentsAtPurchase").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
...
export const orderAssignments = mysqlTable("orderAssignments", {
  id: int("id").autoincrement().primaryKey(),
  orderId: int("orderId").notNull().unique(), // One driver per order
